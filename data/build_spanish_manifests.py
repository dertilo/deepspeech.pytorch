...
def process_example(wav_dir, txt_dir, audio_file, text):
    ...
    subprocess.call(["sox {}  -r {} -b 16 -c 1 {}".format(audio_file, str(SAMPLE_RATE),
...
if __name__ == '__main__':
    ...
    for audio_file,text in tqdm(read_jsonl(base_path+'/spanish_%s.jsonl'%split_type)):
        audio_file = base_path+audio_file
        process_example(wav_dir, txt_dir, audio_file, text)

    create_manifest(wav_dir, 'spanish_' + split_type + '_manifest.csv',
                    1, 30)
